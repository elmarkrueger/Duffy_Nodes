...
* Utilize V3 typed objects for schema definitions, such as `io.Image.Input("id")`, `io.String.Input("id", multiline=True)`, or `io.Float.Input("id", display_mode=io.NumberDisplay.slider)`. Note that the first argument MUST be a unique string ID.
* Define `inputs` and `outputs` in `io.Schema` as lists of typed objects, not dictionaries.
...
* Process image tensors strictly in the `[B, H, W, C]` dimension format (standard ComfyUI layout).
...
class MyCustomNode(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            ...
            inputs=[
                io.Float.Input("base_value", display_name="Base Value", default=1.0, min=0.0, max=100.0, display_mode=io.NumberDisplay.slider),
            ],
            outputs=[
